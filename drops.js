...
const DROP_TYPES = [
    ...
    { 
        ...
        effect: (p) => {
            enemySystem.enemies.forEach(e => { e.hp = 0; createFloatText(e.x, e.y, "XXX", '#e74c3c'); });
    ...
    },
    {
        ...
        onStart: (p) => { p.saveProj = p.projectiles; p.projectiles += 24; p.bulletSize *= 0.5; },
        onEnd: (p) => { p.projectiles = p.saveProj; p.bulletSize /= 0.5; },
        drawIcon: (ctx) => {
...
function spawnShockwave(x, y, color) {
    if (typeof shockwaves !== 'undefined') {
        shockwaves.push({x, y, radius: 5, opacity: 1, color: color});
    }
}